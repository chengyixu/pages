...
const PortfolioAllocation: React.FC = () => {
  ...
  const allocationStrategies = strategies.filter(s => s.type === 'portfolio_allocation');

  // Fetch real portfolio performance data using Polygon API with proper date handling - ONLY portfolio strategies
  const fetchRealPerformanceData = async () => {
    const activePortfolioStrategies = strategies.filter(s => s.isActive && s.type === 'portfolio_allocation');
    if (activePortfolioStrategies.length === 0) {
    ...
    try {
      ...
      const data = await generateRealCombinedPerformanceData(activePortfolioStrategies);
      setPortfolioPerformanceData(data);
    } catch (error) {
  ...
  const generateSectorData = () => {
    if (allocationStrategies.length === 0) return [];
    ...
    allocationStrategies.forEach(strategy => {
      if (strategy.allocations) {
        strategy.allocations.forEach(allocation => {
          ...
          const sector = sectorMapping[allocation.ticker] || '其他';
  ...
  const generateRiskReturnData = () => {
    return allocationStrategies.map(strategy => ({
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200 pb-4">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 chinese-text">投资组合配置</h1>
            <p className="text-gray-600 mt-1 chinese-text">智能资产配置和风险管理系统 - 实时Polygon数据</p>
          </div>
          <button
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={portfolioPerformanceData}>
                ...
                />
                {/* Real Strategy Lines */}
                {strategies.filter(s => s.isActive && s.type === 'portfolio_allocation').slice(0, 4).map((strategy, index) => {
            ...
            <div key={strategy.id} className="chart-clean p-6">
              ...
              {strategy.allocations && strategy.allocations.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <h5 className="text-sm font-medium text-gray-900 chinese-text mb-2">持仓配置</h5>
                  <div className="space-y-1">
                    {strategy.allocations.map((allocation, allocIndex) => (
                      <div key={allocIndex} className="flex justify-between text-sm">
                        <span className="english-text">{allocation.ticker}</span>
                        <span className="english-text">{allocation.weight}%</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {sectorData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="chart-clean p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 chinese-text">行业配置分布</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPieChart>
                  <Pie
      ...
      {/* Show message when no allocation strategies exist */}
      {allocationStrategies.length === 0 && (
        <div className="chart-clean p-6 text-center">
          <p className="text-gray-500 chinese-text">暂无组合配置策略，请在策略列表中添加组合配置策略。</p>
        </div>
      )}
    </div>
  );
